...
export default function Column({ id, title, tasks, teamMembers, onEditTask, onToggleSubtask, onDeleteTask, onArchiveTask, onAddTask }) {
    ...
    >
      ...
      <div className="p-2 lg:p-3 space-y-2 lg:space-y-3 overflow-y-auto flex-1 custom-scrollbar">
        <SortableContext items={tasks.map(t => t.id)} strategy={verticalListSortingStrategy}>
          {tasks.map(task => (
            <TaskCard key={task.id} task={task} teamMembers={teamMembers} onEdit={onEditTask} onToggleSubtask={onToggleSubtask} onDelete={onDeleteTask} onArchive={onArchiveTask} />
          ))}
        </SortableContext>
