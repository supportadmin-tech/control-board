...
export default async function handler(req, res) {
  ...
  try {
    ...
    const LETTERMAN_API_KEY = req.headers.authorization?.replace('Bearer ', '') || null;

    if (!LETTERMAN_API_KEY) {
      return res.status(400).json({ error: 'Letterman API key not configured. Add it in Settings.', noKey: true });
    }
